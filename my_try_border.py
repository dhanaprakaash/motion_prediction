...
path = astar(grid_map, robot_position, robot_goal)


#debug3
print ("init:", robot_position )
print ("goal:", robot_goal)
print ("Human Trajectory: " , path)
...
# Plot the initial position of the robot as a red circle marker with 0.5m step size
robot_marker, = ax.plot([], [], 'ro', markersize=10)  # Red circle marker for robot

# Plot the robot's movement trajectory as a blue line with 0.5m step size
robot_line, = ax.plot([], [], 'b-', lw=2)  # Blue line for robot trajectory

# Update function for animation
def update(frame):
    if frame < len(robot_trajectory):
        x_robot, y_robot = robot_trajectory[frame]
        robot_marker.set_data([y_robot], [x_robot])  # Update robot's position
        robot_line.set_data([pos[1] for pos in robot_trajectory[:frame+1]], [pos[0] for pos in robot_trajectory[:frame+1]])  # Update robot trajectory
        ax.set_title(f'Frame: {frame+1}')  # Set title with frame number
    
    return robot_marker, robot_line

# Animate the robot's movement
ani = FuncAnimation(fig, update, frames=len(robot_trajectory), interval=100, blit=True)
